...
export const cartReducer = (state, action) => {
  switch (action.type) {
    case cartActions.ADD: {
      return {
        cart: [...state.cart, { ...action.product, cartQty: action.qty }],
    ...
    case cartActions.REMOVE_SAVED: {
      const editedSaveForLater = state.saveForLater.filter(
        (x) => x !== action.product
      );
    ...
    case cartActions.SET_QUANTITY: {
      ...
      const editedItem = newCart.find(
        (x) => x.id == action.product.id && x.size == action.product.size
      );
